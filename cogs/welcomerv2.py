...
class Welcomer:
    ...
    async def welcomemsg(self,ctx, args:str = None, args1:str = None):
        '''set the welcome msg  Syntax welcomemsg msg channelid'''
        server = ctx.guild
        url = self.load_url()
    ...
    async def leavemsg(self,ctx, args:str = None, args1:str = None):
        '''set the leave msg  Syntax leavemsg msg channelid'''
        server = ctx.guild
        url = self.load_url2()
            ...
            if "{}".format(server.id) in data_message:
                ...
                if data_type["{}".format(server.id)]["msg_type"] == "pic":
                    try:
                        ...
                        new_im.save("welcome.png")
                        await ctx.send(content = "**This is the preview of the welcome you set.**",file=discord.File('welcome.png'))
                    except Exception as e:
                        print(e)
                ...
                if "{}".format(server) in data_message:
                    ...
                    if data_type["{}".format(server)]["msg_type"] == "pic":
                        ...
                        try:
                            ...
                            bb.text((136, 134),"Bot: {}".format(is_bot), fill="white",font = font2)
                            new_im.save("welcome.png")
                            await ctx.send(content = f"**The Server is {serrv.name}**",file=discord.File('welcome.png'))
                        except Exception as e:
                            print(e)
                            await ctx.send("The image link might be invalid (not a valid .png/.jpg link) :sweat_smile:")
    ...
    async def on_member_join(self,member):
        ...
        try:
            ...
            #    print(url)
                ...
                bb.text((136, 134),"Bot: {}".format(is_bot), fill="white",font = font2)
                new_im.save("welcome.png")
                await channel.send(content = "**Welcome {0.mention}!**".format(member),file=discord.File('welcome.png'))
                try:
